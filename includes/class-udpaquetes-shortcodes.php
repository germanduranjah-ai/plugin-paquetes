...
final class UDPAQUETES_Shortcodes {
    ...
    public static function shortcode_udpq_paquetes_filter($atts = []) {
        ...
        $destino_terms = [];
        if ($show_destino && taxonomy_exists(UDPAQUETES_CPT::TAX_DESTINO)) {
                ...
                <?php if ($show_destino): ?>
                    <div class="udpq-filters__field">
                        ...
                        <?php if (!empty($destino_terms) && !is_wp_error($destino_terms)): ?>
                            <select name="destino">
                                <option value=""><?php esc_html_e('Todos los destinos', 'ud-paquetes'); ?></option>
                                <?php foreach ($destino_terms as $term): ?>
                                    <option value="<?php echo esc_attr($term->slug); ?>" <?php selected($v_destino, $term->slug); ?>>
